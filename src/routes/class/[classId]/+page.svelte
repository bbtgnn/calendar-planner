<script lang="ts">
  ...
  import {
    ...
    invalidateLoadKeys,
    runMutation,
  } from "$lib/kit/runMutation";
  import {
    createLesson,
    updateLesson,
    deleteLessonCascade,
  } from "$lib/application/lessons";
  ...
  $effect(() => {
    const synced = syncAddFormToKind(newSessionKind, {
      hours: newHours,
      title: newTitle,
    });
  ...
  const classNumberByLessonId = $derived.by(() => {
    const map = new Map<LessonId, number>();
    let n = 0;
    for (const lesson of data.lessons) {
      if (lesson.sessionKind === "class") {
        n += 1;
        map.set(lesson.id, n);
      }
    }
  ...
  function handleNewSessionKindChange(event: Event) {
    ...
    const nextKind = (event.currentTarget as HTMLSelectElement)
      .value as LessonSessionKind;
    newSessionKind = nextKind;
    const next = applyKindToForm(prevKind, nextKind, {
      hours: newHours,
      title: newTitle,
    });
    newHours = next.hours;
  ...
  <div class="grid">
    ...
    <label>
      ...
      <input
        ...
        min="0"
        step="0.5"
        bind:value={targetStudentLessonHours}
      />
    </label>
    ...
    <section class="notes-panel" aria-label="Lesson notes">
      <div class="notes-panel__head">
        ...
        <button
          type="button"
          class="btn"
          onclick={() => void refreshNotesFromFolder()}
        >
          Refresh from folder
        </button>
      </div>
      ...
      {:else}
        <p class="muted notes-panel__ok">
          No warnings from lezioni/ or extra/ notes.
        </p>
      {/if}
    </section>
  {/if}
      ...
      <table>
        ...
        <tbody>
          {#each data.lessons as lesson (lesson.id)}
            <tr
              class:upcoming={data.upcomingDate !== null &&
                lesson.date === data.upcomingDate}
              aria-label={data.upcomingDate !== null &&
              lesson.date === data.upcomingDate
                ? "Upcoming session"
                : undefined}
            >
              <td>
                ...
                {#if lesson.sessionKind === "class"}
                  <span class="badge-class-number">
                    ({classNumberByLessonId.get(lesson.id)})
                  </span>
                {/if}
              </td>
              ...
              <td class="done-cell">
                ...
                {:else if lesson.done}
                  ...
                  {#if lesson.hoursWarning}
                    <span
                      class="warn-icon"
                      title="Hours: planner {lesson.hoursWarning
                        .plannerHours}h, note {lesson.hoursWarning.noteHours}h"
                      >⚠</span
                    >
                  {/if}
