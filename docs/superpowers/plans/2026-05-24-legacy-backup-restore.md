# Legacy backup restore — implementation plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a one-off hidden `/restore` page that replaces all Dexie data from a legacy JSON backup file.

**Architecture:** Single disposable Svelte page (`src/routes/restore/+page.svelte`) with inline parse/validate/import logic — no shared module, no unit tests. User picks a file, sees a summary, confirms, then a single Dexie transaction clears and bulk-imports all four tables.

**Tech Stack:** SvelteKit 2, Svelte 5, Dexie (IndexedDB), Bun

**Spec:** `docs/superpowers/specs/2026-05-24-legacy-backup-restore-design.md`
...
|------|--------|----------------|
| `src/routes/restore/+page.svelte` | **Create** | Entire restore UI + validation + Dexie write |

No changes to `+layout.svelte` (hidden route).

---

### Task 1: Create restore page

**Files:**
- Create: `src/routes/restore/+page.svelte`
...
<div class="panel">
	...
	<p class="warn">
		This replaces <strong>all</strong> lesson planner data in this browser. It cannot be undone.
	</p>
...
Run: `bun run check`  
Expected: PASS (no errors in `src/routes/restore/+page.svelte`)
...
Navigate to `http://localhost:5173/restore` (port may differ).

- [ ] **Step 3: Load backup file**

Click **Choose backup file**, select `lesson-planner-legacy-backup-2026-05-24.json`.

Expected summary: **2 classes, 40 lessons, 0 students, 0 absences**.

- [ ] **Step 4: Restore**

Click **Restore**, confirm the dialog.

Expected: toast “Backup restored”, redirect to `/`, class switcher shows “Creative Coding 1” and “Creative Coding 2”.

- [ ] **Step 5: Verify persistence**

Open each class schedule — lessons should appear. Refresh the page — data should remain.
...
- [ ] **Step 1: Commit restore page and spec**

```bash
git add src/routes/restore/+page.svelte docs/superpowers/specs/2026-05-24-legacy-backup-restore-design.md docs/superpowers/plans/2026-05-24-legacy-backup-restore.md
git commit -m "$(cat <<'EOF'
Add one-off /restore page for legacy JSON backup import.

Disposable utility to replace all Dexie data from a legacy export file before the page is removed post-migration.
EOF
)"
```
...
|------------------|------|
| Hidden `/restore` route, no nav link | Task 1 (new route only) |
| Replace all with confirm | Task 1 `onRestore` |
| File picker + summary | Task 1 UI |
| Strip `recordStatus`, defaults for optional fields | Task 1 `parseLessonRow` / `parseClassRow` |
| FK validation | Task 1 `parseBackup` |
| Dexie transaction clear + bulkAdd | Task 1 `onRestore` |
| clearLastClassId + toast + redirect | Task 1 `onRestore` |
| Error messages per spec table | Task 1 `parseBackup` + catch block |
| Manual verification steps | Task 2 |
| No unit tests (non-goal) | — |

## Cleanup reminder

After verifying restored data in production use, delete `src/routes/restore/+page.svelte`.
